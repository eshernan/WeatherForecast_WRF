...
settings['process']['forecast_hours_d01'] = settings['process']['forecast_hours_'+settings['globals']['run_type']+'_d01']
settings['process']['forecast_hours_d02'] = settings['process']['forecast_hours_'+settings['globals']['run_type']+'_d02']
settings['process']['forecast_hours_d03'] = settings['process']['forecast_hours_'+settings['globals']['run_type']+'_d03']
...
settings['process']['end_date_d01'] = settings['globals']['start_date'] + \
                              relativedelta(hours=int(settings['process']['forecast_hours_d01']))
settings['process']['end_date_d02'] = settings['globals']['start_date'] + \
                              relativedelta(hours=int(settings['process']['forecast_hours_d02']))
settings['process']['end_date_d03'] = settings['globals']['start_date'] + \
                              relativedelta(hours=int(settings['process']['forecast_hours_d03']))
...
fecha=format(args.start_date)+"_"+format(args.run_time.zfill(2))
logging.info("Enviando email de resultados")
os.system('/opt/scripts/mail.sh '+fecha+'')
os.system('export SIMULATION_DATE='+fecha)
